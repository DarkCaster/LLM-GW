...
**IMPORTANT:** when writing python code, consider to skip adding any additional config format validation into python code. Lua config validated on load by `PyLuaHelper`. If you need to validate config parameters that is not a runtime check - add lua validation logic into `post.lua` lua script.
